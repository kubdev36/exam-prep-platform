...
class ImportLoigiaihayEnglishCommand extends Command
{
    protected $signature = 'import:loigiaihay-english';
    protected $description = 'Crawl and import all THPT English exams from loigiaihay.com category';

    public function handle(ExamStructureParser $parser)
    {
        $this->info('Starting crawler for loigiaihay THPT English exams...');
        ...
        $categoryUrl = 'https://loigiaihay.com/de-thi-tot-nghiep-thpt-mon-anh-c2203.html';
        $this->line("Fetching category: $categoryUrl");

        $thptType = ExamType::where('code', 'THPT')->first();
        $englishSubject = Subject::where('code', 'THPT_ENGLISH')->first();

        if (!$thptType || !$englishSubject) {
            $this->error('THPT ExamType or THPT_ENGLISH Subject not found in database.');
            return 1;
        }
        ...
        $this->info('Found ' . count($examLinks) . ' exam articles to import.');
        ...
        foreach ($examLinks as $link => $rawTitle) {
            ...
            DB::transaction(function () use (
                ...
                $exam = Exam::create([
                    ...
                    'total_score' => 10.0,
                    'description' => "Đề thi tốt nghiệp THPT môn Tiếng Anh có đáp án và lời giải chi tiết theo chương trình chuẩn.",
                    'attempts_count' => rand(85, 340),
                    'average_score' => round(rand(65, 84) / 10, 1),
                    'is_published' => true,
                ]);
        ...
        $this->info("==================================================");
        $this->info("Import completed: $importedCount exams, $totalQuestionsImported questions imported into PostgreSQL database!");
